Pred_ute.App <- function(X_unsamp,distM,post_samp1,uv,r01,r02)
{
  ...
  for(m in 1:K){
    ...
    for(j in 1:100){
    Ym <- Res_Clcpp(xdata=X_unsamp,distM=distM,para=post_samp1[m,],Z1=t(rnorm(nrow(X_unsamp))),Z2=t(rnorm(nrow(X_unsamp))),uv=uv,r01=r01,r02=r02)
    Y1[j,] <- Ym[,1]
    Y2[j,] <- Ym[,2]
    }
